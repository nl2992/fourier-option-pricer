...
| `hilbert_lookback_price(model, fwd, params, maturity=..., cp=-1, n_monitor=252, h=None, N=None)` | Levy model key, market inputs | Discretely monitored floating-strike lookback via the Lindley recursion with Hilbert projections (Feng-Linetsky 2009); also `method="hilbert_lookback"`. |
