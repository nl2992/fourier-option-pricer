...
- **Fixed:** `bsm_lookback_floating` (the top-level export, also behind `price(..., method="lookback_bsm")`) had a transcription error in the Goldman-Sosin-Gatto correction term (`e^{-qT}` for `e^{-rT}`, `N(a1)` for `N(-a1)`), mispricing at-inception floating lookbacks by ~10% (e.g. put 17.06 instead of 19.42 for S=100, r=5%, q=2%, sigma=25%, T=1; call 23.06 instead of 19.30). It now delegates to the correct, tested `foureng.pricers.analytic_bsm.bsm_lookback_floating`. Found while validating the Hilbert lookback engine: Richardson-extrapolating discrete monitoring to the continuous limit reproduces Haug's closed form to 2e-5. The old tests only checked inequalities; value regressions are added.
- Added model-free variance from option strips (`foureng.analytics.model_free_variance`, exported): `log_contract_variance_from_strip` (log-contract replication `2/(DT) int Q(K)/K^2 dK` over out-of-the-money quotes, Simpson on each side of the forward with the forward inserted as a node), `vix_style_index` (CBOE VIX discretisation), and `log_contract_variance` (the exact model value `-2 c_1 / T`). Strips priced by the contour engine recover BSM `sigma^2` to 1e-8, Heston's expected integrated variance to 1e-7 and Kou's `-2 c_1 / T` to 1e-6; with Kou's jumps the variance-swap strike sits ~10% above the log contract, the jump correction the docs call out.
- Contour engine: only strikes that have not converged are refined further, CF evaluations are chunked, and convergence stops at the attainable accuracy (eps times the integrand's peak size), so wide strips stay fast (801 strikes in ~0.4 s). The 4/2 CF's Kummer series now uses a streaming log-sum-exp (memory linear in the number of frequencies instead of terms x frequencies).
- Added a high-precision reference engine, `method="contour"` in `price_strip` (`contour_price_at_strikes`, `ContourGrid`, exported). For each strike the Lord & Kahl (2007) optimal contour height (the saddle point of the payoff-transform integrand) is chosen across the three residue regions, so out-of-the-money values come out residue-free with full *relative* precision; the semi-infinite integral uses an adaptive exp-sinh double-exponential rule (as advocated by Andersen & Lake 2018). The admissible contour strip is found by scanning the moment generating function outward and stopping at non-finite, non-real or non-convex values, which also catches CFs that return finite garbage beyond a moment explosion (VG with integer `T/nu`). BSM agrees with the closed form to 5e-14 relative out to six standard deviations for T from 0.01 to 5, and to 1e-13 for 1e-34/1e-39 prices; VG agrees with an independent Gamma-mixture quadrature to 5e-13 relative down to 1e-19 prices; the price is independent of the contour to 1e-12 for Heston, VG, CGMY, Kou, NIG and 4/2; refined COS (N = 4096) and fine Hilbert agree to 5e-13. It also shows the default `cos_improved` grid carries ~1e-10–1e-9 absolute error on these models.
- Added the 4/2 stochastic-volatility model (Grasselli 2017) as registry model `"sv42"` (`Sv42Params(v0, kappa, theta, nu, rho, a, b)`, `sv42_cf`, `sv42_cumulants`): volatility `a sqrt(v) + b / sqrt(v)` on a single CIR factor, mixing Heston and 3/2 smile dynamics with a volatility floor `2 sqrt(ab)`. The closed-form CF is derived by conditioning on the variance path, a Girsanov change of mean-reversion speed that absorbs `int v`, and a change of CIR dimension that absorbs `int 1/v`, leaving a noncentral-chi-square moment given by a Kummer function; the Kummer series is summed in log space with the Poisson factor folded in, so no complex-parameter special-function library is needed (~1 ms per 100 frequencies). Works with every CF engine (COS, improved/filtered COS, Hilbert, FFT/FRFT, CONV, PROJ European). Tests: `b = 0` equals Heston (including when Feller fails) and `a` rescales Heston variance, `a = 0` equals the 3/2 model through `V = b^2/v`, both to 1e-14; martingale normalisation; 30-digit mpmath agreement to 5e-13 down to T = 0.01; exact-transition conditional Monte Carlo with a martingale control variate for four `(a, b, rho)` sets; engine agreement; parameter validation (Feller condition enforced when `b > 0`).
- Added American options for every supported 1-D Lévy model: `method="cos_american"` in `price()` and `cos_american_price` (exported). Four-point Richardson extrapolation of COS Bermudans with `m, 2m, 4m, 8m` exercise dates (Fang & Oosterlee 2009, section 5), default `m = 64`, floored at the finest Bermudan and at immediate exercise; the term count adapts per level to the CF decay over the exercise interval. BSM puts/calls agree with Richardson-extrapolated 40k-step binomial trees to 1e-6–1.5e-5; Kou and CGMY agree with the same extrapolation over the independent PROJ Bermudan engine to 2e-6/2e-5. Previously American pricing was BSM-only.
- `cos_bermudan_price` is now the exact Fang–Oosterlee (2009) scheme: early-exercise point by safeguarded Newton on the COS series, analytic payoff coefficients, and continuation coefficients through the Hankel + Toeplitz matrix applied by FFT. It replaces a spatial-grid/trapezoid approximation whose payoff-kink error was ~7e-5 even in the European limit; the new scheme matches European COS to 1e-9, converges exponentially in `N`, and is 20–80x faster. The `n_spatial` argument is deprecated and ignored (emits `DeprecationWarning`). The PROJ-vs-COS Bermudan cross-check tolerances tighten from 1e-3 to 1e-9 (BSM) / 1e-6 (Kou) / 2e-5 (CGMY) / 1e-4 (VG, singular short-step density), after widening the PROJ lattice, whose 1x cumulant width had cost it ~1e-3 on Kou.
- Added `implied_vol_lets_be_rational` and `black_price`: a vectorised NumPy implementation of Jäckel's (2015) "Let's Be Rational" Black implied-volatility algorithm. Every quote is reduced to a normalised out-of-the-money call; a rational-cubic initial guess on one of four branches (lower map, two middle interpolants around the inflection point `sqrt(2|x|)`, upper map) is refined by order-3 Householder steps on branch-specific objectives inside a bisection safeguard. The normalised Black function avoids cancellation through a small-`t` series (vega integrated term by term, with a continued-fraction Mills ratio), a scaled-erfc form, and an exact upper complement. On 200k random OTM quotes: max relative vol error 4e-15, two iterations suffice for every quote, ~0.7 µs/option versus ~430 µs for `implied_vol_newton_safeguarded`. Far-wing quotes down to 1e-250 reprice to <1e-12 against 50-digit mpmath. `model_iv_surface` (and therefore every `calibrate_*` objective) and `foureng.utils.implied_vol.implied_vol_from_prices` now use it instead of per-cell Newton/Brent loops. Tests: round trip on a 20k OTM grid, the two-iteration property, mpmath backward-error and normalised-Black checks, Brent agreement, ITM parity reduction, broadcasting, boundary/invalid quotes, surface repricing. `mpmath` joins the `[test]` extra as a high-precision oracle.
