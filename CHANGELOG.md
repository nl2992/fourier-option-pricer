...
Two-asset options (Hurd and Zhou 2010), with `method="fourier_2d"`:

- Spread, exchange and two-asset best-of options, and a new `RainbowOption` (call or put on the max or min of two assets).
- Two-asset models `bsm2d`, `vg2d` (both assets on one gamma clock) and `heston2d` (one variance for both assets), with their joint CF in `joint_cf`. With `model="bsm"` the product's `sigma2` and `rho` are used.
- Prices agree with an independent quadrature to about 1e-12 relative for BSM and VG. The grid adapts to the CF, so short-dated, highly correlated spreads stay fast, and a warning is raised if it hits its size limit.
- `RainbowOption` can also be priced by `multi_asset_mc`.

Calibration:

- `calibrate(model, quotes, initial)` fits any registry model with float parameters to option quotes (`MarketQuotes`, vols or prices, any strikes and maturities). It uses vega-weighted out-of-the-money prices, bounded least squares and a Jacobian built from CF gradients. On a 45-quote Heston surface it recovers the parameters from a poor start in 18 evaluations (0.02 s) with an IV error of 6e-11; `calibrate_heston` needs 675 evaluations for 7e-8. Parameters can be fixed, bounded and weighted.
- `cf_and_gradient`: analytic CF gradients for BSM, Merton, Kou, VG, NIG, CGMY, Heston and Bates, and differences of the CF for the other models.

Exotics under stochastic volatility, with `method="cos_ctmc"` (Cui, Kirkby and Nguyen 2018):

- Bermudan, American and discretely monitored barrier options under Heston, Bates and the regime-switching model. The variance becomes a Markov chain and the log-price is handled by the COS method.
- Heston American puts agree with the Ikonen and Toivanen benchmark to 1e-5, Europeans with the closed form to about 1e-6 (also when the Feller condition fails), and barriers with Monte Carlo. The regime-switching model is priced exactly.
- If `threadpoolctl` is installed, the engine keeps BLAS single-threaded while it builds its transition matrices. Without it, `OPENBLAS_NUM_THREADS=1` avoids a large slowdown seen on some 4-core Linux machines.

### Fixed

- `sv32` gave wrong prices at short maturities: PyFENG's CF series for the 3/2 model cancels catastrophically (|phi| reached 1e118 at T = 0.1), so SINC and contour priced calls below intrinsic. `sv32_cf` now evaluates the same closed form in-house and agrees with mpmath to about 1e-13 for maturities from 0.005 to 10 years. It raises an error if |phi(u)| ever exceeds 1 for real u.
- The API reference described the Margrabe and Kirk payoffs with the assets swapped.

### Changed

- CI test jobs run with single-threaded OpenBLAS, time out after 25 minutes and print stack traces for tests that run over 4 minutes.
...
Models (all usable with every CF engine through `price_strip`):

- `sv42`: the 4/2 stochastic volatility model of Grasselli (2017). Setting `b = 0` gives Heston and `a = 0` gives the 3/2 model.
- `bns`: Barndorff-Nielsen and Shephard (2001) Gamma-OU stochastic volatility, with variance jumps that also move the price.
- `lifted_heston`: Abi Jaber's (2019) multi-factor Markovian approximation of rough Heston.
- `time_changed_levy`: a BSM, VG, NIG, CGMY, Kou or Merton process run on a CIR or Gamma-OU clock (Carr, Geman, Madan and Yor 2003). VG on a CIR clock is the existing `vgsa`.
- `affine`: generic affine jump-diffusions in any dimension (Duffie, Pan and Singleton 2000), solved from the Riccati equations. Heston, Bates, double Heston and Merton can all be written this way. A state-dependent jump compensator has to go in `K1`.

European pricing engines:

- `method="contour"`: Lord and Kahl (2007) optimal contour with double-exponential quadrature. Out-of-the-money prices keep their relative precision however small they get, so it works as a reference engine. It assumes a cheap CF, so use COS or SINC for `affine` and `lifted_heston`.
- `method="sinc"` and `sinc_smile`: the SINC method of Baschetti et al. (2022). It is the same sum as the existing Hilbert engine, but with a window sized from the cumulants, so it needs far fewer terms. `sinc_smile` prices a whole smile with one FFT.
- `method="swift"`: the Shannon wavelet method of Ortiz-Gracia and Oosterlee (2016).

Exotics for 1-D Levy models:

- `method="cos_american"` / `cos_american_price`: American options by Richardson extrapolation of COS Bermudans (Fang and Oosterlee 2009). Until now American pricing was BSM only.
- `method="hilbert_barrier"` / `hilbert_barrier_price`: discretely monitored single barriers by the fast Hilbert transform (Feng and Linetsky 2008).
- `method="hilbert_lookback"` / `hilbert_lookback_price`: discretely monitored floating- and fixed-strike lookbacks (Feng and Linetsky 2009).
- `method="asian_cos"` / `levy_arithmetic_asian_price`: fixed-strike arithmetic Asians (Zhang and Oosterlee 2013). These were Monte Carlo only before.

Implied volatility and analytics:

- `implied_vol_lets_be_rational` and `black_price`: a vectorised version of Jaeckel's (2015) "Let's Be Rational" solver. It is accurate to machine precision and takes about 0.7 microseconds per option. `model_iv_surface`, and so every `calibrate_*` function, now uses it.
- `log_contract_variance_from_strip`, `vix_style_index` and `log_contract_variance`: model-free variance and a VIX-style index from option quotes, plus the matching model value.

### Fixed

- `bsm_lookback_floating` (the top-level function, also used by `price(..., method="lookback_bsm")`) had the wrong correction term and was about 10% off for new contracts. For example, the put came out at 17.06 instead of 19.42 for S=100, r=5%, q=2%, sigma=25%, T=1. It now uses the tested formula in `foureng.pricers.analytic_bsm`.
- The LICENSE file is now included in the sdist and wheel.

### Changed

- `cos_bermudan_price` now implements the Fang and Oosterlee (2009) scheme exactly instead of a spatial-grid approximation. It is more accurate (it now matches the European price when there is one exercise date) and 20 to 80 times faster. Results change slightly as a result. The `n_spatial` argument is deprecated and ignored.
- `matplotlib` is no longer installed by default. Install `fourier-option-pricer[viz]` if you use `foureng.viz`. `statsmodels` stays a dependency because `pyfeng` imports it.
- The package ships type information (`py.typed`) and supports Python 3.10 to 3.14.
- `foureng.__version__` is read from `foureng/_version.py`.
- The release workflow checks that the tag matches the version and runs the tests before uploading to PyPI.
...
- Added the exact discrete geometric-Asian pricer for Levy models (Fusai & Meucci 2008) as `method="asian_cf"`: the average's CF is the finite product of per-increment CFs at scaled frequencies, so no lognormal approximation. Matches the discrete Kemna-Vorst BSM closed form to 1e-8 and Kou Monte Carlo within the confidence band. `levy_geometric_asian_price` exported.
