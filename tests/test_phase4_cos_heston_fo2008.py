...
def test_cos_heston_fo2008_L_sensitivity(fo2008_heston):
    """Truncation-L sensitivity: with enough N, the price is stable across L.

    For L in {6, 8, 10, 12} at N=512, the price should vary by < 1e-7. This
    demonstrates the COS truncation error is controlled and not materially
    affecting the result once N is large enough.
    """
    d = fo2008_heston
    fwd = ForwardSpec(S0=d["S0"], r=d["r"], q=d["q"], T=d["T"])
    p = HestonParams(kappa=d["kappa"], theta=d["theta"], nu=d["nu"],
    ...
    assert spread < 1e-6, f"L-sensitivity too high at N=512: {prices} spread={spread:.3e}"
