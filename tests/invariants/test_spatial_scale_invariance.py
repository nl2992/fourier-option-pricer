...
def test_atm_spatial_scale(model, params, lam):
    ...
    K0 = _BASE_FWD.S0
    fwd_scaled = fe.ForwardSpec(S0=lam * _BASE_FWD.S0, r=_BASE_FWD.r, q=_BASE_FWD.q, T=_BASE_FWD.T)
...
def test_strip_spatial_scale(model, params):
    ...
    np.testing.assert_allclose(
        p_scaled, lam * p_base, rtol=1e-4, err_msg=f"{model}: strip scale invariance failed"
    )
