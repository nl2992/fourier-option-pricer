...
def test_levy_bermudan_exceeds_european(model, params, fwd):
    ...
    product_eur = BermudanOption(strike=100.0, maturity=1.0, cp=-1, exercise_times=np.array([1.0]))
    product_4 = BermudanOption(
        strike=100.0, maturity=1.0, cp=-1, exercise_times=np.linspace(0.25, 1.0, 4)
    )
    p_eur = cos_bermudan_price(model, fwd, params, product_eur)
    p_4 = cos_bermudan_price(model, fwd, params, product_4)
    assert p_4 >= p_eur - 1e-6, f"{model}: M=4 Bermudan {p_4:.6f} < European {p_eur:.6f}"
...
def test_levy_bermudan_positive_finite(model, params, fwd):
    ...
    for K in [90.0, 100.0, 110.0]:
        product = BermudanOption(
            strike=K, maturity=1.0, cp=-1, exercise_times=np.linspace(0.25, 1.0, 4)
        )
        price = cos_bermudan_price(model, fwd, params, product)
        assert np.isfinite(price) and price >= 0.0, f"{model} K={K}: price={price}"
...
def test_levy_bermudan_monotone_in_frequency(model, params, fwd):
    ...
    assert prices[0] <= prices[1] + 1e-6, f"{model}: M=1 {prices[0]:.6f} > M=4 {prices[1]:.6f}"
    assert prices[1] <= prices[2] + 1e-6, f"{model}: M=4 {prices[1]:.6f} > M=12 {prices[2]:.6f}"
