...
from foureng.models.bsm import bsm_cash_or_nothing, bsm_asset_or_nothing
...
def test_vanilla_decomposition_from_digitals(model, params, K):
    ...
    aon = cos_digital_price(model, fwd, params,
                            DigitalOption(strike=K, maturity=1.0, cp=1,
                                          payoff_type="asset_or_nothing"))
    con = cos_digital_price(model, fwd, params,
                            DigitalOption(strike=K, maturity=1.0, cp=1,
...
def test_cash_or_nothing_put_call_sum_equals_discount(model, params):
    ...
    for K in [90.0, 100.0, 110.0]:
        con_call = cos_digital_price(model, fwd, params,
                                     DigitalOption(strike=K, maturity=1.0, cp=1,
                                                   payoff_type="cash_or_nothing"))
        con_put = cos_digital_price(model, fwd, params,
                                    DigitalOption(strike=K, maturity=1.0, cp=-1,
                                                  payoff_type="cash_or_nothing"))
...
def test_asset_or_nothing_bounded_by_spot(bsm_fwd, bsm_params):
    ...
    strikes = np.linspace(70.0, 130.0, 13)
    for cp in (1, -1):
        prices = cos_digital_price_strip("bsm", fwd, bsm_params, strikes, 1.0, cp=cp,
                                         payoff_type="asset_or_nothing")
...
def test_cash_amount_scales_price(bsm_fwd, bsm_params):
    ...
    product_1 = DigitalOption(strike=100.0, maturity=1.0, cp=1,
                               payoff_type="cash_or_nothing", cash_amount=1.0)
    product_5 = DigitalOption(strike=100.0, maturity=1.0, cp=1,
                               payoff_type="cash_or_nothing", cash_amount=5.0)
...
def test_strip_matches_scalar_loop(bsm_fwd, bsm_params):
    ...
    for i, K in enumerate(strikes):
        product = DigitalOption(strike=float(K), maturity=1.0, cp=1,
                                payoff_type="cash_or_nothing")
