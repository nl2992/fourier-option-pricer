...
def test_down_out_call_decreases_as_barrier_rises():
    ...
    prices = [
        bsm_barrier_price(S, K, H, cp=1, barrier_type="down_out", **_PARAMS) for H in barriers
    ]
...
def test_up_out_put_decreases_as_barrier_falls():
    ...
    prices = [bsm_barrier_price(S, K, H, cp=-1, barrier_type="up_out", **_PARAMS) for H in barriers]
    assert all(prices[i] >= prices[i + 1] - 1e-8 for i in range(len(prices) - 1)), (
...
def test_down_out_call_converges_to_vanilla_at_small_barrier():
    ...
    assert abs(p_low_H - vanilla) < 1e-5, f"H→0: DOC={p_low_H:.6f} should ≈ vanilla={vanilla:.6f}"
...
def test_up_out_put_converges_to_vanilla_at_large_barrier():
    ...
    assert abs(p_high_H - vanilla) < 1e-5, f"H→∞: UOP={p_high_H:.6f} should ≈ vanilla={vanilla:.6f}"
...
def test_down_out_call_specific_value():
    ...
    doc = bsm_barrier_price(S, K, H, r=r, q=q, T=T, sigma=sigma, barrier_type="down_out", cp=1)
    dic = bsm_barrier_price(S, K, H, r=r, q=q, T=T, sigma=sigma, barrier_type="down_in", cp=1)
