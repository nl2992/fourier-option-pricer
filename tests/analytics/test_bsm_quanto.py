...
def test_forward_rho_zero_reduces_to_simple_drift():
    ...
    F = bsm_quanto_forward(
        S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
        rho=0.0, sigma_S=0.20, sigma_X=0.10, T=T
    )
...
def test_forward_negative_rho_lower_than_zero_rho():
    ...
    F_neg = bsm_quanto_forward(S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
                                rho=-0.5, sigma_S=0.20, sigma_X=0.10, T=1.0)
    F_zero = bsm_quanto_forward(S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
                                 rho=0.0, sigma_S=0.20, sigma_X=0.10, T=1.0)
    assert F_neg > F_zero


def test_forward_positive_rho_lower_than_zero_rho():
    """Positive rho decreases effective drift: -rho*sigma_S*sigma_X < 0 when rho>0."""
    F_pos = bsm_quanto_forward(S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
                                rho=0.5, sigma_S=0.20, sigma_X=0.10, T=1.0)
    F_zero = bsm_quanto_forward(S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
                                 rho=0.0, sigma_S=0.20, sigma_X=0.10, T=1.0)
    assert F_pos < F_zero
...
def test_forward_zero_maturity():
    F = bsm_quanto_forward(S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
                            rho=-0.3, sigma_S=0.20, sigma_X=0.10, T=0.0)
...
def test_forward_formula_exact():
    ...
    F = bsm_quanto_forward(
        S=100, r_dom=0.05, r_for=0.03, q_for=0.02,
        rho=-0.3, sigma_S=0.20, sigma_X=0.10, T=1.0
    )
    assert abs(F - expected) < 1e-12
...
def test_parity_at_base_params():
    ...
    F = bsm_quanto_forward(
        S=BASE['S'], r_dom=BASE['r_dom'], r_for=BASE['r_for'],
        q_for=BASE['q_for'], rho=BASE['rho'],
        sigma_S=BASE['sigma_S'], sigma_X=BASE['sigma_X'], T=BASE['T']
    )
...
def test_parity_itm_call():
    ...
    F = bsm_quanto_forward(
        S=kw['S'], r_dom=kw['r_dom'], r_for=kw['r_for'],
        q_for=kw['q_for'], rho=kw['rho'],
        sigma_S=kw['sigma_S'], sigma_X=kw['sigma_X'], T=kw['T']
    )
...
def test_zero_sigma_S_call_itm():
    ...
    F = bsm_quanto_forward(
        S=kw['S'], r_dom=kw['r_dom'], r_for=kw['r_for'],
        q_for=kw['q_for'], rho=kw['rho'],
        sigma_S=0.0, sigma_X=kw['sigma_X'], T=kw['T']
    )
...
def test_raises_negative_S():
    with pytest.raises(ValueError, match="S must be positive"):
        bsm_quanto_forward(S=-1, r_dom=0.05, r_for=0.03, q_for=0.02,
                            rho=0.0, sigma_S=0.2, sigma_X=0.1, T=1.0)
